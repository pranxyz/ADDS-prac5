...
int main()
{
    ...
    vector<int> L_1 = mapAbsoluteValue->map(mapTriple->map(*L));

    FilterOdd *filterOdd = new FilterOdd();
	FilterForTwoDigitPositive *filterForTwoDigitPositive = new FilterForTwoDigitPositive();
    vector<int> L_2 = filterOdd->filter(filterForTwoDigitPositive->filter(L_1));

    vector<int> *L_2_vec = new vector<int>();
    for (vector<int>::iterator it = L_2.begin(); it != L_2.end(); it++)
